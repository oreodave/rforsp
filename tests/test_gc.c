...
static void test_live_object_survives(void)
{
  ...
  /* live should still be a valid pair, dead should be reclaimed
   * (we check this indirectly: after collection, the pointer for dead
   *  may be in the free list — but we can still allocate and verify) */
  pair_t *p = (pair_t *)UNTAG(live);
...
static void test_chain_survives(void)
{
  ...
  pair_t *hp = (pair_t *)UNTAG(head);
  TCHECK(hp->car == mid, "head.car is mid");
  pair_t *mp = (pair_t *)UNTAG(mid);
...
static void test_multiple_collections(void)
{
  ...
  for (int round = 0; round < 3; ++round)
  {
    ...
    mkpair(NULL, NULL); /* garbage */
    mkpair(NULL, NULL); /* garbage */

    /* After the first round, the previous keep is still allocated
     * (allocated but unmarked), so count is 1 + 3 = 4 in later rounds. */
...
static void test_closure_cycle_garbage(void)
{
  ...
  obj_t *env  = mkpair(clos, NULL); /* env contains reference to clos */
  obj_t *body = mkpair(clos, NULL); /* body also references clos */
  ((clos_t *)UNTAG(clos))->body = body;
  ((clos_t *)UNTAG(clos))->env  = env;
